...
module Interval : Interval = struct
  ...
  let rec add a b = 
    ...
    | Range(MinusInf , Int n2), Range(Int n3 , Int n4) -> Range(MinusInf, Int(n2+n4))
    | Range(Int n1 , PlusInf), Range(Int n3 , Int n4) -> Range(Int(n1+n3), PlusInf)
    | Range(Int n1, Int n2), Range(MinusInf , Int n4) -> Range(MinusInf, Int(n2+n4))
    | Range(Int n1 , Int n2), Range(Int n3 , PlusInf) -> Range(Int(n1+n3), PlusInf)
  ...
  let rec sub a b = 
    ...
    | Range(Int n1 , PlusInf), Range(Int n3 , Int n4) -> (Range(Int(n1-n4), PlusInf))
    | Range(Int n1, Int _), Range(MinusInf , Int n4) -> (Range(Int(n1-n4), PlusInf))
    | Range(Int n1 , Int n2), Range(Int n3 , PlusInf) -> (Range(MinusInf, Int(n2-n3)))
...
= fun node m -> 
  ...
  | I_assign(lv, e) -> 
    ...
    if BatSet.is_singleton (loc_set) then 
      (
        BatSet.fold (fun elt acc -> AbsMem.strong_update elt abs_val m) loc_set m
      )
    else
      (
...
(* assume you’ve already loaded all the modules and definitions above,
       including `transfer : Node.t -> AbsMem.t -> AbsMem.t` and `test_mem` *) 

(* ─── I_alloc ───────────────────────────────────────────────────────────────── *)

let n_alloc = Node.create_alloc "x" 5;;
let mem_alloc = transfer n_alloc AbsMem.empty;;  
let _ = AbsMem.print mem_alloc;;

(* mem_alloc now binds:
     Var "x"     ↦ ( Interval.bot,
                      AbsArray.create l [5,5] )
     Allocsite l ↦ ( Interval.Range(Interval.Int 0,Interval.Int 0),
                      AbsArray.bot )
   where l is the fresh allocsite produced by `n_alloc`. *)

(* ─── I_assign (strong update) ──────────────────────────────────────────────── *)

(* start from a memory where i ↦ ([0,0], bot) *)
let mem0 = AbsMem.add (AbsLoc.Var "i") (Interval.Range(Interval.Int 0,Interval.Int 0), AbsArray.bot) AbsMem.empty;;

let n_assign = Node.create_assign (ARR("arr", NUM 1)) (NUM 7);;

let mem_assign = transfer n_assign mem0;;  
let _ = AbsMem.print mem_assign;;

(*  unchanged. -> arr is not defined *)

let n_assign2 = Node.create_assign (ID "i") (NUM 3);;
let mem_assign2 = transfer n_assign2 mem0;;  
let _ = AbsMem.print mem_assign2;;

(* strong update on variable i:
     Var "i" ↦ ( Interval.Range(Interval.Int 3,Interval.Int 3),
                 AbsArray.bot )
   no allocsite bindings are touched. *)

(* ─── I_skip ────────────────────────────────────────────────────────────────── *)

let n_skip = Node.create_skip ();;
let mem_skip = transfer n_skip mem0;;  
let _ = AbsMem.print mem_skip;;

(* skip does nothing: mem_skip = test_mem *)

(* ─── I_read ────────────────────────────────────────────────────────────────── *)

let n_read = Node.create_read "j";;
let mem_read = transfer n_read mem0;; 
let _ = AbsMem.print mem_read;;

(* read has no abstract effect: mem_read = test_mem *)

(* ─── I_print ───────────────────────────────────────────────────────────────── *)

let n_print = Node.create_print (ADD (NUM 2, NUM 5));;
let mem_print = transfer n_print mem0;;  
let _ = AbsMem.print mem_print;;

(* print has no abstract effect: mem_print = test_mem *)

(* ─── Combined check examples ───────────────────────────────────────────────── *)

(* Example: allocate y, then assign it, then skip *)
let mem_chain =
  let m1 = transfer (Node.create_alloc "y" 3) AbsMem.empty in
  let m1 = transfer (Node.create_alloc "y" 4) m1 in
  let m2 = transfer (Node.create_assign (ARR("y", NUM 0)) (NUM 1)) m1 in
  let m3 = transfer (Node.create_skip ())           m2 in
  m3
;;  

let _ = AbsMem.print mem_chain;;
